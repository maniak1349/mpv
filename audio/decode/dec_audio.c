...
void audio_work(struct dec_audio *da)
{
    ...
    if (segment_end && da->new_segment) {
        ...
        da->current_state = DATA_AGAIN;
    } else if (0 && segment_end) {
        // Make it discard cached/unused data.
        da->segment_ended = true;
        if (da->ad_driver)
            da->ad_driver->control(da, ADCTRL_RESET, NULL);
    }
}
